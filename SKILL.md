...
**根本限制** (v4.7.2, v4.7.7修正, v4.7.8更新): 文本规则无法打破LLM验证强迫——模型进入吸引子状态时执行检查的认知能力已被循环劫持。可靠性排序: **L2平台阻断(echo-guard.js v3.3+stop-guard.js v3.2双hook+permissions.deny备份层) > L3一字恢复 > L1文本规则**。模型侧最佳努力: 完成实质操作(Write/Edit/Agent启动)后立即停止turn，不确认状态、不echo确认词、不Read已写入文件。用户下一条消息时自然重置。

**外部看门狗** (v4.7.8, echo-guard.js v3.3 + stop-guard.js v3.2): 双hook注册于 settings.json。
...
- **Stop hook** `stop-guard.js` v3.2 (转移包§七: Stop=对抗审查唯一可靠入口): turn结束时解析transcript末400行——最后一次assistant turn含≥2确认词且零Write/Edit/Agent调用→`{decision:"block"}`注入"写snapshot+输出⚡进度行后停止"。stall门(干净轮<2未发射Agent→block)+饱和豁免(/⚡[^"]{0,80}\/compact/共现)+clean-exit门(收敛声明须同turn落snapshot, 认"干净轮2/2"/"## 签收单"/"签收单:"/"最终签收单")+task-notification边界不作userText(\bstop\b词边界)。`stop_hook_active=true`必须放行(每stop最多1次干预, 平台级防hook自循环)。覆盖PreToolUse结构性盲区: 无工具调用的纯确认turn。
