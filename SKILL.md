...
# code-shiniyaya v4.7.10 — 平台层防御+外部加速Skill+五层验证管线

v4.7.10 — 五层验证管线落地(L1静态→L2 AI初审→L3对抗验证→L4清单驱动→L5人工核验)。v4.7.9基础之上: 转移包§八-Q2 5层验证体系全量串入, L1-L3变更前强制门控, L4 Quadruple 28规则↔20自检内化映射, L5=STEP 5已有门控。CC 请求-响应架构：Claude 不能给自己发消息、不能自主启动新 turn、不能在压缩后自动续跑。全自动做不到。能做到的：保存全部状态 → 用户只需打一个字"继" → 任务无缝恢复。fake full-auto that silently fails < honest semi-auto that reliably works.
...
- (d) **auto-clarity 5条件**：安全警告、不可逆操作确认、多步序列碎片顺序歧义、压缩本身产生技术二义性、用户要求澄清或重复提问→写完整语言。结束后恢复
- (e) 级别切换：用户"stop caveman"→§34暂停；"resume caveman"→恢复
- (f) 不自称：不输出"[CAVEMAN]"标签，不宣布风格

### §35 触发守卫（原using-superpowers）
- 任何操作前：先检查code-shiniyaya §触发词表(L318-338)是否匹配
- 匹配则强制调用skill工作流
- 假阳性门控：裸词"双重检查"/"交叉审计"不含"codex"前缀→不触发
- 13条红旗思维问题（触发前自问参见触发词表L321-338）：需求是否明确、测试环境是否可用、是否直接操作生产、是否有回滚计划、影响范围是否确定、是否缺乏监控、时间压力是否跳过步骤、是否存在单点故障、是否依赖未验证外部数据、是否绕过安全机制、是否未与相关方确认、是否缺乏文档记录、是否疲劳状态。红牌≥3→先确认再执行
...
- 30条硬规则中的规则1-14, 18-20, 26: 门控/Agent/Plan-Code Gap/停止线/用户中断/推理空间/死循环阻断
- 反模式1-9, 18, 24: 核心编码和验证纪律+产出路径散落检查(自检#16同级)+Codex手动粘贴循环
...
**根本限制** (v4.7.2, v4.7.7修正, v4.7.8更新): 文本规则无法打破LLM验证强迫——模型进入吸引子状态时执行检查的认知能力已被循环劫持。可靠性排序: **L2平台阻断(echo-guard.js v4.3+stop-guard.js v3.5双hook+permissions.deny备份层) > L3一字恢复 > L1文本规则**。模型侧最佳努力: 完成实质操作(Write/Edit/Agent启动)后立即停止turn，不确认状态、不echo确认词、不Read已写入文件。用户下一条消息时自然重置。

**外部看门狗** (v4.7.8, echo-guard.js v4.3 + stop-guard.js v3.5 + bearings.js v3.0-r9): 三hook注册于 settings.json。
- **PreToolUse Bash hook** `echo-guard.js` v4.3: stdin JSON读取tool_input.command → (1)拦截无意义echo(6词+空串) (2)同turn同文件wc -l第2次拦截 (3)8次非只读Bash/turn上限(READONLY token-array Set数据结构: grep/rg/cat/head/tail/wc/uniq/stat无条件豁免; find/sort/diff为READONLY_WITH_POTENTIAL_FLAGS——先默认豁免再检查DESTRUCTIVE_FLAGS Set(-delete/-exec/-execdir/-fprint[0f]/-fls/-ok/-okdir/--output)和sort专用SORT_ONLY_DESTRUCTIVE(-o/-O)。command-context-aware v4.1: find的-o是逻辑OR(非破坏性), sort的-o是输出文件(破坏性)——区分对待。--output=FILE equals-form已处理 (4)30秒非豁免调用空闲窗口≈turn边界(双时间戳: _capTs仅非豁免调用刷新, lastTs所有调用用于wcFiles/count重置) (5)指纹阶梯(v4.2: 归一化文件参数——rm a.mp4/rm b.mp4→同指纹rm <FILE>, 防参数变体绕过, cross-turn, 15min TTL, 仅小写命令词归一化) (6)**v4.3: 所有阻断路径统一使用deny()/ask()辅助函数输出hookSpecificOutput.permissionDecision——PreToolUse handler需要此格式方可实际拒绝执行**(Scan37 P1) (7)逐级升级: 同指纹第2次→systemMessage; 第3次→ask; ≥4次→deny+换策略指令。
- **Stop hook** `stop-guard.js` v3.5 (转移包§七: Stop=对抗审查唯一可靠入口): turn结束时解析transcript末400行——最后一次assistant turn含≥2确认词且零Write/Edit/Agent调用→`{decision:"block"}`注入"写snapshot+输出⚡进度行后停止"。stall门(干净轮<2未发射Agent→block)+**pre-launch门**(v3.4: turn含"第N轮→继"声明但无Workflow/Agent/Task调用→block一次)+饱和豁免(/⚡[^"]{0,80}\/compact/共现)+clean-exit门(收敛声明须同turn落snapshot, 认"干净轮2/2"/"## 签收单"/"签收单:"/"最终签收单")+task-notification边界不作userText(\bstop\b词边界, v3.5移除裸"报告"(过宽))。**v3.5关键修复**(Scan35 P0): tool_result行break(防跨turn污染——前一turn的Agent/Write泄漏到当前turn绕过pure-confirmation+pre-launch门); CONFIRM仅匹配text块(排除tool_use/tool_result块中的done/ok膨化)。`stop_hook_active=true`必须放行(每stop最多1次干预, 平台级防hook自循环)。覆盖PreToolUse结构性盲区: 无工具调用的纯确认turn。
...
**已评估拒绝** (v4.7.8, 转移包§三评估+转移包§四/补充包完整扫描): ①claude-focus——每调用注入变动前缀→prompt cache全未命中, 任务锚定已由iteration-task.md+snapshot承担→拒绝。②LoopLens/unloop-mcp整装安装——MCP依赖违背零依赖hook原则, unloop的指纹+升级模式已内化为echo-guard v3→拒绝安装, 模式采纳。③Semantic Early-Stopping原样实现——无embedding API→降级为规则24词法重叠。④asyncRewake心跳唤醒——后台Agent完成本就自动唤醒, sleep+rewake=重新发明轮询器→拒绝。⑤headroom_compress——文件重定向严格更优(内容不进上下文 vs 压缩副本+原件双份)→拒绝。⑥PAR整体替换"10+ Agent验证"——该块承载grounding协议/re-prompt/规则7/规则20契约, ≥6处下游引用, 替换波及大于收益→拒绝(PAR保留为独立review场景用)。**v4.7.9 批量补充**: ⑦impeccable 100+反模式——UI/设计反模式(渐变/WCAG/8pt网格), 与验证编排器领域不匹配→拒绝。⑧Quadruple Verification 28规则——冗余(现有20项自检+manual-verification-checklist.md覆盖)→拒绝。⑨MCP/平台依赖类(promptfoo/Mem0/debug-mcp/Langfuse/Guardrails/LangGraph/Temporal/quality-guardian/SonarQube/Danger/token-optimizer-mcp/cc2cc/claude-mem/cozempic)——模式已由零依赖路径吸收(上下文注入→bearings.js, 基线门控→agent-lint, 对抗→pantheon/MMAR), 批量拒绝, 未来不再单列复述→拒绝。⑩TDD/property-based-testing生成测试→已由hooks.test.js证明可执行测试捕获bug的价值, §STEP 7已含variant-analysis可选slot; standing slot→拒绝(保留为STEP 6 caller端可选invoke以免scope creep). ⑪knip/jscpd/radon/smartdiff CLI预扫→aislop已覆盖确定性预扫slot; 追加仅当目标仓类型出现aislop盲区→拒绝。⑫DSPy签名重构→R1已杀为化妆术(缓存前缀纪律+Finding Provenance JSON已覆盖负载子集), ledger条目省略→本条正式补充。

**台账收敛约束** (v4.7.9-r5): 台账已闭合于12项。依⑨"未来不再单列复述"条款, 已批量拒绝的类(MCP/平台依赖/CLI预扫/测试框架)不再重新发现。任何新增须证明:(a)该项不属已拒绝类的未覆盖子项 AND (b)零依赖路径无法实质替代 AND (c)对echo循环/上下文饱和/卡顿/恢复/收敛判定有直接杠杆。缺任一条件→驳回, 不录入台账。
...
AI: 读最新snapshot → 恢复全部状态 → 继续执行
     ...
     └─ 恢复决策(v4.7.8, 替代模糊的"从断点继续"; 分支①-④自上而下求值首中即行, nextAction与实际状态矛盾时以实际状态(pending/计数)为准):
        ...
        └─ ④ nextAction=verify OR 干净轮计数≥2 → 确认收敛(=按规则24跑最终50 Agent验证, P0=0方可)→写goal-reached-{当前版本}.md→输出签收单(首行"## 签收单", 见上方L185格式)+同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2); **goal-reached-{当前版本}.md已存在且含P0=0字段**→直接签收单——旧版本goal-reached*.md(无版本名/他版本)一律不作数, 不得短路最终验证(v4.7.9-r4: v4.0.2陈旧goal-reached.md曾在HEAD上构成旁路陷阱)
```
...
**一字恢复标准化启动检查清单** (v4.7.6, 来自autonomous-coding coding_prompt.md GET YOUR BEARINGS; v4.7.8增hook步骤8):
当"继"触发恢复时，在执行snapshot恢复后，CC应立即确认bearings.js已注入的环境信息(步骤1-6及journal.jsonl末段——合并turn结束后到达的Agent结果), 并执行步骤7(版本交叉比对)和步骤8(合并journal结果):
1. `pwd` → 确认工作目录
...
`# ponytail: upgrade条件已达成(v4.7.8)——步骤1-6由SessionStart hook(bearings.js, matcher: startup|resume|clear|compact, v4.7.9补clear——ponytail claude-codex-hooks.json同款, /clear后自动重注入)自动注入为系统上下文, 步骤7(版本交叉比对)仍由模型在"继"时执行。hook缺失→回退手动7步。ceiling: hook只消除信息收集成本, 恢复执行仍需用户"继"(CC不能自启turn)`
...
**为什么不自动`/compact`?** 见§记忆保存+一字恢复((correct) 架构现实段)——`/compact`是终端命令, Claude不能向自己发送; ScheduleWakeup/CronCreate均不能替代。任何绕过声明=伪功能。
...
| 70-90% | **禁用扫描发射+自动压缩**: 不调用新Agent; 保存snapshot+git commit(不push)。平台层已设`settings.json` `autoCompactThreshold: 55`(v4.7.9-r2用户定案——上下文≥55%时平台自动触发压缩; 模型侧55%启发式保存与其互补: 模型触发保存, 平台触发压缩。**勿改回70**)。理论上平台55%先触发, 模型走到70-90%档=平台压缩未生效(代理透传/版本问题)→输出"⚡ 70% — /compact + 继" → 终止turn。注: autoCompactThreshold为CC v1.0.57+平台参数, 版本未达→回退60%/80%三级纯模型降级(保存+等用户手动/compact) |
...
1. **写snapshot草稿**: 基于当前已知状态写memory/snapshot-{ts}.md(不要等Agent完成后才写→那时可能已进入echo循环)。草稿必含nextAction(按55%流程四态赋值: Agent在飞/预发射未处理→await; 已入修复(pending非空)→fix; 干净轮计数≥2→verify; 其余→scan)+当前干净轮计数——恢复决策树的两个输入必须在草稿期即落盘。
...
**Codex通信**: 手动复制粘贴 (codex-plugin-cc 未安装/未使用)。
...
**恢复触发词**: 见§一字恢复触发词表——"继"/"继续执行"/"继续迭代"/"继续优化"→snapshot恢复; "继续修复"/"resume"→session JSON恢复。裸词"继续"不触发(过于宽泛)。静默等待: "继续等"/"继续等待"→保持静默计数, 不触发恢复路由。
...
1. **不中断+预发射** (v4.7.5 降级, v4.7.8消除干净轮卡顿, v4.7.9-r6 防伪发射): 同turn内工作流完成→立即处理结果→修复→**若未收敛→同turn内立即启动下一轮扫描Agent(不等用户"继")→然后输出进度行+等结果**。不可跨turn自主启动(CC架构: 无自主跨turn执行)。**中立方执行顺序**(v4.7.9-r7, 诊断#7: 防止"已commit=已完成"认知闭合在Agent发射前触发): **(A) Workflow/Agent调用(实际发射Agent) → (B) snapshot写入 + git commit(状态落盘——Agent已飞, commit是保险不是句号) → (C) 进度行输出(对已执行动作的声明)**。三个步骤缺一不可——A 是动作, B 是保险, C 是声明。禁止 AB 倒序(先 commit 后发射→"已完成"认知闭合)+禁止 AC 跳 B (伪发射, stop-guard v3.5 pre-launch门でblock)。**三硬门**: 启动前过 (i)饱和≥55%→走饱和优先线 (ii)预算剩余<20→走预算耗尽路径 (iii)有在飞/未处理结果→不重复发射先处理。Agent启动后才输出进度行结束turn。**反伪发射规则**(v4.7.9-r6, 审计实证: r4 turn写"第9轮在飞"但未调Workflow): Agent调用必须先于进度行——进度行是对"已实际发射Agent"的声明, 不是发射的替代品。同一turn无Workflow/Agent/Task工具调用记录却含"第N轮…→继"=伪发射——stop-guard v3.4 pre-launch门将block(一次)。预发射失败证明(API错误/Agent启动返回值)必须写入snapshot, 进度行引用之——不得裸输出进度行当作"已完成一轮"。`# ponytail: 预发射Agent结果跨turn可靠性=CC Agent生命周期保证, ceiling: Agent超时/CC重启→回退原"继→发射"路径, upgrade: CC保证跨turn结果可靠投递`
2. **不结束**: 直到 (a)零bug收敛(**干净轮计数器≥2**, 见规则24), (b)用户说停/stop, (c)预算耗尽
3. **不等待** (v4.7.5 降级): 同turn内绝不在迭代中途输出"要继续吗?"。turn结束=停止——等用户"继"恢复。
4. **每轮输出**: 一行进度。进度行仅在前置条件满足(本轮已至少启动一轮≥batch_size个Agent调用且首批已完成)后方可输出——进度行是对已实际发射Agent的声明, 不是发射的替代(v4.7.9-r6, 审计#5)。正常轮: "第N轮[dim]: X→Y→继"(X=发现数, Y=修复数, dim=本轮维度名/干净轮状态——两者二选一填一个); 干净轮: "第N轮[规模]: 干净轮i/2→继"(i=当前计数; 规模=本轮扫描Agent数如20A——规模归一审计依据, v4.7.9)。不输出长篇分析。turn结束符——用户"继"时下轮结果已就绪可直接处理。**Agent全部PERMANENTLY_FAILED时**(v4.7.9-r6, 审计#7): 输出"第N轮[FAIL]: Agent全部PERMANENTLY_FAILED→停"(不含"继"——无在飞Agent可恢复)+原因摘要+写ERRORS.md。**预发射失败**(turn token不足/Agent启动被平台拒绝——仅此两种情况, 非模型自主跳过): 输出"第N轮: X→Y→继"(简式——X/Y须来自上轮实际数据, 不得编造; 失败证明写入snapshot, 禁止裸输出进度行)(v4.7.9-r6, 审计#2简式精确定义)。
...
- "继" → snapshot恢复 → **snapshot中`nextAction`字段决定恢复后首个动作("await"=有在飞/未处理预发射结果, 走恢复决策分支①/"scan"=启动下一轮扫描/"fix"=从pending项继续修复/"verify"=进入验证)。scan→在同turn内启动扫描Agent, 启动后输出进度行; 启动失败→回退fix或verify→仍失败→输出进度行+等用户"继"。**`# ponytail: nextAction字段规范级(手工写入snapshot), ceiling: 恢复动作依赖模型读取+执行(非自动), upgrade: 同bearings.js自动注入一起自动化`
...
## 五层验证管线 (v4.7.10, 转移包§八落地)

每次变更(修复/新增/重构)在进入STEP 5用户批准前, 必须通过L1-L3; L4为CC内化清单(非外部工具); L5=STEP 5门控(已有)。管线执行顺序: 变更完成→L1→L2→L3→L4→L5(STEP 5)→STEP 6执行。任一层失败→回STEP 2重方案。外部skill不可用时每层有明确回退路径。
...
- `node references/hooks.test.js` — hooks/echo-guard/stop-guard自测, 42用例全绿(当前v4.7.10=42, 历史: 30→33→35→38→42)

**失败动作**:
- aislop新增HIGH/CRITICAL→本轮修复标BLOCKED, 回STEP 2重方案
- agent-lint分数下降→进入回滚评估(基线优先), 修复标LINT_REGRESS
- hooks.test任一FAIL→禁止进入干净轮计数(前置条件, §24 收敛阈值自调整 L543)

**交叉引用**: §24 收敛阈值自调整(干净轮前置条件) | §自主迭代模式-item 1(预发射三硬门) | §迭代自检-6 | §STEP 1-确定性预扫 L834(aislop)

### L2 AI初审 — 6+ Agent并行 + diff审查

**触发时机**: L1通过后、STEP 4(Codex审查)前。
...
**交叉引用**: §硬规则-规则20(P0验证规格化, L539) | §硬规则-规则7(失败替换+3层重试) | §反模式-7(单Agent盲信) | §Agent Safety-Three-Layer Defense(L1060)

### L3 对抗验证 — pantheon + PAR/MMAR

**触发时机**: L2通过后、STEP 7双向验证。

**检查方法**:
| 路由 | 条件 | 机制 |
|------|------|------|
| pantheon-fix | 复杂P0 + DAG空 + 用户opt-in | 多候选修复+隔离worktree+回归门控+对抗验证一步完成 |
| MMAR降级 | Codex不可用 + MMAR可用 | 跨模型对抗审查——产出视同Codex粘贴回复走双向验证 |
| PAR | 可用时 | 独立批评者Agent主动攻击提议, 循环至收敛或max轮次 |
| pantheon-gap | P0修复后 + 可用 | 扫描同根因变体bug, 写入VARIANTS报告 |

**失败动作**: pantheon-fix回归门控失败→回滚worktree, 项回STEP 2。MMAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L1000)。PAR不可用→跳过(MMAR兜底)。对抗验证发现新P0→追加pending, 本轮干净轮清零。

**交叉引用**: §STEP 7-双向验证(L990) | §STEP 6-分支路由(pantheon-fix, L942) | §STEP 7-降级模式(MMAR, L1000) | §外部加速Skill(8挂点, (correct))

### L4 清单驱动 — Quadruple 28规则 ↔ 20自检映射

**触发时机**: L3通过后、STEP 5用户批准前。CC内化执行, 零外部工具依赖——Quadruple Verification不安装, 模式内化为现有自检映射。

**检查方法** (28规则→20自检映射, 4周期):

| Quadruple周期 | 对应自检 | 检查内容 |
|---------------|----------|---------|
| 安全性(Q1-Q7) | 自检#7(工作流时间预算)+#17(防卡死) | 安全护栏6项(信任边界/数据丢失/安全措施/无障碍/校准/显式要求); 脚本安全性 |
| 完整性(Q8-Q14) | 自检#5+#6+#8+#9+#10+#12+#14 | 工作流完整性; 7方向覆盖; 模式实际应用(非仅文档化); 5源深度利用 |
| 正确性(Q15-Q21) | 自检#13(产出物写入)+#16(报告路径) | 方案-代码一致性; 修复有效性; 去重(非重复发现); 产出路径合规 |
| 幻觉(Q22-Q28) | 自检#2(不等待)+#18(同turn部分) | 无伪发射(Agent调用先于进度行); 收敛条件真实满足(干净轮≥2); 无编造数据 |

**失败动作**: 任一周期的≥2项自检未通过→该修复轮不计入干净轮; L4未通过项写入snapshot的L4-gaps字段, 下轮L2审查维度追加对应焦点。

**交叉引用**: §迭代自检(20项完整定义, L567) | §24 收敛阈值自调整(干净轮计数器, L543) | §26 无意义输出循环阻断(L552) | §转移包§六(增量验证+审查门控) | §转移包§八-Q2(5层验证体系)

### L5 人工核验 — 用户批准门控

**触发时机**: L1-L4全部通过后。

**检查方法**: STEP 5双批准门控(已有, 此处串入管线)。用户审查L1-L4通过证据→批准执行。Codex双向验证(STEP 4→STEP 7)为人工核验提供第二意见。

**失败动作**: 用户拒绝或Codex拒绝→回STEP 2重方案。用户批准+Codex不可用→降级模式(STEP 5降级条款: 用户单批准可执行, §规则1双批准门控 (correct))。

**交叉引用**: §规则1双批准门控(L502) | §STEP 4-Codex审查 | §STEP 5-门控+用户批准 | §错误处理-降级模式(STEP 5行)

### 管线硬性约束

- **L1为干净轮前置条件**: hooks.test全绿 + 自检#21 Scope无遗漏 + 自检#22 无注入风险——三者缺一→该轮不计干净轮(§24 收敛阈值自调整 (correct))。自检#21+#22完全内化, 无不可用风险, 不阻塞
- **L1-L3为L4输入**: 只有通过前置层的变更才进入清单核对——跳过前置层的清单核对=形式主义
- **L5不可跳过**: 迭代模式规则15暂停规则4(逐项反馈), 但P0安全敏感修复仍需用户中断确认(§15 迭代不中断 (correct))
- **外部skill回退**: pantheon/MMAR/PAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L1000); Quadruple不安装→本表内化映射
...
**自研引用脚本** (位于 `references/`): 以下脚本由本项目的迭代过程创建, 不依赖外部skill:
- **diagnose.sh**: git bisect + 性能剖析诊断工具, 用法: `bash references/diagnose.sh [旧提交] [新提交]`
- **semgrep-style-selfcheck.sh**: semgrep风格的SKILL.md结构自检脚本, 检查规则完整性/交叉引用/格式规范, 用法: `bash references/semgrep-style-selfcheck.sh`
- **generate-charts.py**: Matplotlib图表生成器, 产出SKILL.md五层验证管线流程图、规则依赖关系图等, 用法: `python references/generate-charts.py`

**交叉引用**: §STEP 7-双向验证((correct)) | §外部加速Skill((correct)) | §20 P0验证规格化((correct)) | §迭代自检#10(5源深度利用)

## 硬规则 (30条: 核心规则1-26 + 交付规则27-28 + 质量门控规则29-30, v4.7.10)
...
15. **迭代不中断** (v4.7.7 恢复完整定义): 迭代模式下持续执行不停顿。**停止例外**: (a)零bug收敛达标 (b)用户显式stop (c)致命错误无法继续 (d)预算完全耗尽(>90%+BREAK_GLASS, 见§修复预算)。此规则优先于规则1(双批准门控)、规则4(逐项反馈)、规则9(代码不符回滚)和规则10(方案锁定)——但不覆盖规则2(P0逻辑变更仍需用户批准)和§31(f) (correct)(6项永不简化)中的安全护栏。安全关键的门控检查和P0逻辑变更即使在迭代模式下也不可跳过。turn结束时输出进度行"第N轮: X→Y→继"(自检#1例外d)。
...
**干净轮前置条件** (v4.7.8): 本轮扫描Agent按自检#6规格全部成功返回(失败/TIMED_OUT槽位经规则7处理后再计); scan-state `scannedFiles`与上轮重叠<80% 或显式标注"轮换空间已尽", 否则该轮不计(覆盖盲区→自检#15轮换排空检测); 计入前hooks.test.js全绿(用例数随版本增长, v4.7.10=42(历史基线: 初始=30, 各版本增长至33→35→38→42))+agent-lint分数不降(机器证据附进度行)。**轮换重叠**(v4.7.9-r8 P1-2: scan-state.json 未自动化, 轮换排空退化为 scanner 显式标注"轮换空间已尽"+"离线 audit agent 验维度历史"——扫描轮维度由 scanner agent 记录进 snapshot, 承载体: snapshot 维度清单 > scan-state.json): 当 scanner 判定本轮维度与前2轮重叠≥80%时→显式写"轮换空间已尽"于进度行→该轮不计干净轮
...
计数器持久化: 随每份snapshot落盘(格式"干净轮计数: i/2"——见(correct) snapshot模板, bearings STATE正则双格式容错); 恢复时缺失/不可辨→按0处理(安全方向: 宁多扫一轮, 不误宣收敛)。
**turn-end统一决策** (v4.7.8, 权威——in-turn与"继"恢复共用, 消除双规范分叉): ①pending非空→继续修复; ②否则干净轮<2→启动下一轮扫描; ③否则未跑最终验证→启动50 Agent最终验证; ④50 Agent确认P0=0→写goal-reached-{版本}.md→输出签收单(首行"## 签收单", 见L187格式)→同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2, 含哨兵行)→停(此"停"仅在④全条件满足时有效——不可被①②③路径借用, v4.7.9-r6, 审计#4)。⑤ ②/③不可执行(三硬门全阻断/Agent启动全失败/预算不足)→写snapshot(nextAction保留当前状态)→输出对应状态进度行(饱和=⚡格式含/compact, 预算=停格式, PERMANENTLY_FAILED=第N轮[FAIL]→停)——进度行后停止, 不强行发射(v4.7.9-r6, 审计#6)。修复应用→干净轮清零→回②。§一字恢复四分支(①-④)=本决策的nextAction快捷入口, 冲突时以本决策为准。
...
**交叉引用**: L1自检#21(Scope完整性)和#22(注入防护)为L1干净轮前置条件, 见L399-400。

标记NON-VIABLE的自检项(#1/#3/#4/#11)依赖用户触发恢复后补充检查——不可自纠正。其余可行自检项违反时=立即自纠正。

### 交付 (v4.6.9 统一编号: 规则27-28) + 质量门控 (v4.7.10: 规则29-30)
...
29. **契约前置+验证驱动** (v4.7.10, 转移包§四TDD模式落地): 每个P0/P1修复必须配验证用例——hooks域→hooks.test.js回归用例, SKILL.md/规则域→独立验证脚本。Before-any-edit: Read目标文件(规则11已有)。After-edit-before-done: 运行验证命令——修复未通过验证=未完成。No fix without a checkable invariant。测试文件=AI agent最可靠的需求文档(VibeTDD混合模式)——hooks.test.js是SKILL.md的规范性证明。Fix的done状态由验证命令exit code=0定义, 不是Write工具调用返回(Write成功≠修复正确)。P2修复鼓励验证, 不强求。Clean轮前置条件中的hooks.test.js全绿=此规则的聚合关。

30. **修复后全站交叉一致性审计** (v4.7.10-r13, 实证: echo-guard v3.4→v4.1跨8轮才收敛——因每次修复只改发现点、不查其余引用): 任何修复改变共享值(版本号/计数/hook名称/路径/阈值)后，必须执行跨文件一致性审计——否则该修复视为未完成。审计三步: **(A) 确定权威源**——哪个文件的值是正确的(如echo-guard.js L2的`v4.1`是代码权威源)。**(B) Grep全站引用**——搜索该值的所有出现(`grep -rn "echo-guard v[0-9]" .` / `grep -rn "hooks.test [0-9][0-9]/" .`跨SKILL.md+README+CHANGELOG+memory/*.md)。**(C) 差异→批量修正**——每个非权威引用要么对齐权威源、要么标注"故意不同+原因"。审计范围: 当前值+相近模式(如数字序列30→33→35→38全站必须一致)。修复后commit message列出所有被修改的引用行号——不列=没收尾。Agent扫描发现跨文件版本不一致→标记P1(非P2)——因为它是可被规则30防止的已知bug类。`# ponytail: 三步手动grep→批量Edit, ceiling: 新增引用文件被漏扫, upgrade: 预注册引用索引(版本号→所有包含文件:行清单)自动grep`
...
| 24 | **Codex手动粘贴循环** (v4.7.1) | codex-plugin-cc未集成, 仍用手动复制粘贴→等待Codex回复→粘贴回CC→验证→等待→... 9步闭环中STEP 3/4/5全部依赖用户手工介入, 迭代速度受限于人类复制粘贴延迟 | 回退: 手动复制粘贴 (codex-plugin-cc未安装/未使用) |
...
触发 → 路由 (短语匹配优先于状态匹配):
  ...
  ├─ 恢复触发词("继"/"继续执行"/"继续修复"等) → 详见§一字恢复触发词表
...
**输入** (注入LLM): workflow_context_bus 快照 (diagnosis + plan + codex + execution + meta), 现有记忆文件 (最多24个, 每个≤2500字符, 60/40截断), MEMORY.md 索引 (≤6000字符)。**自产工件排除——限bug扫描输入** (v4.7.9, autodream L582-584/L1119 tag-and-filter): bug扫描/诊断Agent的输入排除本skill自产工件(报告/、snapshot-*、CHANGELOG)——读取端按来源过滤, 事前预防自消费回授。本Phase(反思)不受此条约束: 记忆文件/MEMORY.md是Learn的法定输入(delete/upsert/去重判定依据), Phase 2 Consolidate同理
...
**做**: 多源文件后诊断/6+ Agent(5类型)/精确old→new代码/ast.parse验证/报告写入正确路径/纯文本格式/逐项反馈/符号影响分析/遵守全部30条规则/Codex静默→跳过选项/stop→立即停→保存JSON状态。调参查阅 memory/config.json。
...
- **安全护栏**: 详见§31(f) (correct)——6项永不简化。即使梯级1-6认为可以简化, 安全代码不可移除。
...
| 自检#18 死循环根因阻断 | §迭代自检-18 | 同turn内追踪工具调用+跨turn内容hash比对 | **部分可行** | 同turn内(a)(b)可行; 跨turn(c)(d)不可行(CC每turn fresh context——Bash层重复命令子集由echo-guard v4.3跨turn指纹覆盖, 纯确认turn由stop-guard拦截) |
...
**文本规则无法打破LLM验证强迫吸引子状态。** 规则26在模型正常决策时通过transcript自查可执行。但当模型进入Read→Grep→wc -l→Write done→Read done的验证循环时，执行预调用检查的认知能力本身已被循环劫持——v4.6.5的规则26在死循环发生时就已存在，加强措辞到v4.6.9不改变这一根本机制。唯一有效的解决方案是外部看门狗（per-turn工具调用预算、监督模型、进程级循环检测），这是SKILL.md文本无法创建的——**已由echo-guard.js v4.3+stop-guard.js v3.5实现Bash层+turn终态阻断(见§外部看门狗)**; Read/Grep/Write工具层仍无hook覆盖。

**自检#18跨turn部分模型侧不可行。** "跨turn内容hash比对"要求CC模型维护跨turn持久状态。CC每turn从fresh context启动，仅transcript中的历史工具调用可见——但transcript不包含结构化的工具调用元数据（时间戳、调用栈、hash），模型无法可靠计算自检#18(c)(d)所需的指标。同turn内(a)(b)仍可行(v4.7.5降级); Bash层跨turn由echo-guard v4.3指纹承接(v4.7.8)。
...
`# ponytail: 旧selftest缺失声明已被v4.7.6 Selftest双层门控取代(见§自应用差距-Selftest双层门控已定义 L1687)。保留此行作为历史锚点——ponytail对自己的judge.py执行selftest，code-shiniyaya现已对自身规则执行selftest。`
