...
**根本限制** (v4.7.2, v4.7.7修正, v4.7.8更新): 文本规则无法打破LLM验证强迫——模型进入吸引子状态时执行检查的认知能力已被循环劫持。可靠性排序: **L2平台阻断(echo-guard.js v4.1+stop-guard.js v3.4双hook+permissions.deny备份层) > L3一字恢复 > L1文本规则**。模型侧最佳努力: 完成实质操作(Write/Edit/Agent启动)后立即停止turn，不确认状态、不echo确认词、不Read已写入文件。用户下一条消息时自然重置。

**外部看门狗** (v4.7.8, echo-guard.js v4.1 + stop-guard.js v3.4): 双hook注册于 settings.json。
- **PreToolUse Bash hook** `echo-guard.js` v4.1: stdin JSON读取tool_input.command → (1)拦截无意义echo(6词+空串) (2)同turn同文件wc -l第2次拦截 (3)8次非只读Bash/turn上限(READONLY token-array Set数据结构: grep/rg/cat/head/tail/wc/diff/uniq/stat无条件豁免; find/sort为READONLY_WITH_POTENTIAL_FLAGS——先默认豁免再检查DESTRUCTIVE_FLAGS Set(-delete/-exec/-execdir/-fprint[0f]/-fls/-ok/-okdir/--output)和sort专用SORT_ONLY_DESTRUCTIVE(-o/-O)。command-context-aware v4.1: find的-o是逻辑OR(非破坏性), sort的-o是输出文件(破坏性)——区分对待。--output=FILE equals-form已处理 (4)30秒非豁免调用空闲窗口≈turn边界(双时间戳: _capTs仅非豁免调用刷新, lastTs所有调用用于wcFiles/count重置) (5)指纹阶梯(cross-turn, 15min TTL, 仅小写命令词归一化) (6)逐级升级: 同指纹第2次→systemMessage; 第3次→ask; ≥4次→deny+换策略指令。
- **Stop hook** `stop-guard.js` v3.4 (转移包§七: Stop=对抗审查唯一可靠入口): turn结束时解析transcript末400行——最后一次assistant turn含≥2确认词且零Write/Edit/Agent调用→`{decision:"block"}`注入"写snapshot+输出⚡进度行后停止"。stall门(干净轮<2未发射Agent→block)+**pre-launch门**(v3.4: turn含"第N轮→继"声明但无Workflow/Agent/Task调用→block一次)+饱和豁免(/⚡[^"]{0,80}\/compact/共现)+clean-exit门(收敛声明须同turn落snapshot, 认"干净轮2/2"/"## 签收单"/"签收单:"/"最终签收单")+task-notification边界不作userText(\bstop\b词边界)。`stop_hook_active=true`必须放行(每stop最多1次干预, 平台级防hook自循环)。覆盖PreToolUse结构性盲区: 无工具调用的纯确认turn。
...
- `node hooks.test.js` — hooks/echo-guard/stop-guard自测, 38用例全绿(当前v4.7.10=38)
