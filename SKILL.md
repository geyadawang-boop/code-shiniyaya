...
- `node hooks.test.js` — hooks/echo-guard/stop-guard自测, 44用例全绿(当前v4.7.10=44, 历史: 30→33→35→38→44)

**失败动作**:
- aislop新增HIGH/CRITICAL→本轮修复标BLOCKED, 回STEP 2重方案
- agent-lint分数下降→进入回滚评估(基线优先), 修复标LINT_REGRESS
- hooks.test任一FAIL→禁止进入干净轮计数(前置条件, §硬规则-规则24 L429)

**交叉引用**: §硬规则-规则24(干净轮前置条件) | §自主迭代模式-item 1(预发射三硬门) | §迭代自检-6 | §v4.6.11-aislop(确定性预扫, L714)
...
**交叉引用**: §硬规则-规则20(P0验证规格化, L423) | §硬规则-规则7(失败替换+3层重试) | §反模式-7(单Agent盲信) | §Agent Safety-Three-Layer Defense(L940)
...
**失败动作**: pantheon-fix回归门控失败→回滚worktree, 项回STEP 2。MMAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L880)。PAR不可用→跳过(MMAR兜底)。对抗验证发现新P0→追加pending, 本轮干净轮清零。

**交叉引用**: §STEP 7-双向验证(L870) | §STEP 6-分支路由(pantheon-fix, L825) | §STEP 7-降级模式(MMAR, L880) | §外部加速Skill(8挂点, L70)
...
**交叉引用**: §迭代自检(20项完整定义, L451) | §硬规则-规则24(干净轮计数器, L427) | §硬规则-规则26(无意义输出循环阻断, L436) | §转移包§六(增量验证+审查门控) | §转移包§八-Q2(5层验证体系)
...
**失败动作**: 用户拒绝或Codex拒绝→回STEP 2重方案。用户批准+Codex不可用→降级模式(STEP 5降级条款: 用户单批准可执行, §硬规则-规则1 L387)。

**交叉引用**: §硬规则-规则1(双批准门控, L386) | §STEP 4-Codex审查 | §STEP 5-门控+用户批准 | §错误处理-降级模式(STEP 5行)

### 管线硬性约束

- **L1为干净轮前置条件**: hooks.test全绿 + agent-lint分数不降 + aislop无新增HIGH/CRITICAL——三者缺一→该轮不计干净轮(§硬规则-规则24 L429)。外部工具不可用时跳过(重依赖自检#5+#6人工比对), 不阻塞
- **L1-L3为L4输入**: 只有通过前置层的变更才进入清单核对——跳过前置层的清单核对=形式主义
- **L5不可跳过**: 迭代模式规则15暂停规则4(逐项反馈), 但P0安全敏感修复仍需用户中断确认(§硬规则-规则15例外条款 L418)
- **外部skill不可用的回退**: aislop/agent-lint不可用→跳过(重依赖自检#5+#6人工比对); pantheon/MMAR/PAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L880); Quadruple不安装→本表内化映射; 所有回退不阻塞管线, 仅降低对应层置信度
...
**turn-end统一决策** (v4.7.8, 权威——in-turn与"继"恢复共用, 消除双规范分叉): ①pending非空→继续修复; ②否则干净轮<2→启动下一轮扫描; ③否则未跑最终验证→启动50 Agent最终验证; ④50 Agent确认P0=0→写goal-reached-{版本}.md→按L351模板输出签收单(首行"## 签收单")→同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2, 含哨兵行)→停(此"停"仅在④全条件满足时有效——不可被①②③路径借用, v4.7.9-r6, 审计#4)。⑤ ②/③不可执行(三硬门全阻断/Agent启动全失败/预算不足)→写snapshot(nextAction保留当前状态)→输出对应状态进度行(饱和=⚡格式含/compact, 预算=停格式, PERMANENTLY_FAILED=第N轮[FAIL]→停)——进度行后停止, 不强行发射(v4.7.9-r6, 审计#6)。修复应用→干净轮清零→回②。§一字恢复四分支(①-④)=本决策的nextAction快捷入口, 冲突时以本决策为准。
...
`# ponytail: 旧selftest缺失声明已被v4.7.6 Selftest双层门控取代(见§自应用差距-Selftest双层门控已定义, L1565)。保留此行作为历史锚点——ponytail对自己的judge.py执行selftest，code-shiniyaya现已对自身规则执行selftest。`
