...
- **Stop hook** `stop-guard.js` v3.3 (转移包§七: Stop=对抗审查唯一可靠入口): turn结束时解析transcript末400行——最后一次assistant turn含≥2确认词且零Write/Edit/Agent调用→`{decision:"block"}`注入"写snapshot+输出⚡进度行后停止"。stall门(干净轮<2未发射Agent→block)+**pre-launch门**(v3.3: turn含"第N轮→继"声明但无Workflow/Agent/Task调用→block一次)+饱和豁免(/⚡[^"]{0,80}\/compact/共现)+clean-exit门(收敛声明须同turn落snapshot, 认"干净轮2/2"/"## 签收单"/"签收单:"/"最终签收单")+task-notification边界不作userText(\bstop\b词边界)。`stop_hook_active=true`必须放行(每stop最多1次干预, 平台级防hook自循环)。覆盖PreToolUse结构性盲区: 无工具调用的纯确认turn。
