...
# code-shiniyaya v4.7.5 — 一字恢复：诚实面对CC架构极限

v4.7.5 — honest compaction model. CC 请求-响应架构：Claude 不能给自己发消息、不能自主启动新 turn、不能在压缩后自动续跑。全自动做不到。能做到的：保存全部状态 → 用户只需打一个字"继" → 任务无缝恢复。fake full-auto that silently fails < honest semi-auto that reliably works.
...
15. **迭代不中断** (v3.9.1, v4.7.5 降级): 同turn内扫描→修复→重扫必须连续执行。工作流完成→立即处理结果+修复+启动下一轮（同turn内继续）。绝不在迭代中途停下来汇报。~~跨turn自主启动不可行(CC架构: 无自主跨turn执行)~~→ v4.7.5替代: turn结束输出"第N轮: X→Y→继"→用户"继"→从snapshot恢复继续。仅在以下情况暂停: (a) 零bug+零gap+零优化机会(达标停止→触发确认), (b) 用户显式说"停"/"stop"/"报告", (c) 同一文件连续3次修复失败(规则12), (d) 预算完全耗尽(含BREAK_GLASS 10%)。**此规则优先于规则1(双批准门控)、规则4(逐项反馈)、规则9(代码不符回滚)和规则10(方案锁定)——迭代模式下门控降级为事后审计(CHANGELOG.md), 执行不等待批准。例外: P0安全敏感修复(数据丢失/安全漏洞/权限提升)仍需在CHANGELOG.md审计记录后触发用户中断确认。** `# ponytail: 跨turn自主迭代不可行(CC无事件循环)，ceiling: 同turn内连续+一字恢复跨turn，upgrade: CC支持自主跨turn执行时`
...
2. **不等待检查** (v4.7.5 降级): CC是否在等用户回复才继续? 若当前turn内可继续→立即处理结果+继续。若turn已结束→输出"第N轮: X→Y→继"→等用户"继"恢复。
3. **不静默检查** (v4.7.5 标记NON-VIABLE): CC是否在工作流完成后退出了? 若退出→等用户"继"触发恢复→读取最后journal.jsonl+应用修复+启动下一轮。
4. **工作流存活检查** (v4.7.5 标记NON-VIABLE): 上一个工作流是否被kill/崩溃/超时? 若是→用户"继"恢复时解析journal.jsonl部分结果→应用已确认的修复→仅重跑失败维度。
5. **循环持续检查** (v4.3.3 分层): 同turn内检查——分两阶段: (a) 先触发5b(2轮无新类别+CRITICAL不变→切未读文件), (b) 若5b不满足但3轮趋同速率(CR)<20%或CRITICAL不降→切换Agent类型+维度优先级(规则24收敛标准: 发现数<5且连续2轮<10且均为P1/P2→触发最终验证, 确认P0=0则目标达成)。达标→输出达标确认+等用户批准。
...
| 落后检测 | §Agent编排-StreamFirst | 3+ Agent完成，某Agent无log()事件 | **不可行** | 需要live log()事件流+跨Agent完成对比 |
