...
- 13条红旗思维问题（触发前自问参见触发词表L321-341）：需求是否明确、测试环境是否可用、是否直接操作生产、是否有回滚计划、影响范围是否确定、是否缺乏监控、时间压力是否跳过步骤、是否存在单点故障、是否依赖未验证外部数据、是否绕过安全机制、是否未与相关方确认、是否缺乏文档记录、是否疲劳状态。红牌≥3→先确认再执行
...
**为什么不自动`/compact`?** 见§记忆保存+一字恢复(L162 架构现实段)——`/compact`是终端命令, Claude不能向自己发送; ScheduleWakeup/CronCreate均不能替代。任何绕过声明=伪功能。
...
- hooks.test任一FAIL→禁止进入干净轮计数(前置条件, §24 收敛阈值自调整 L546)

**交叉引用**: §24 收敛阈值自调整(干净轮前置条件) | §自主迭代模式-item 1(预发射三硬门) | §迭代自检-6 | §STEP 1-确定性预扫 L837(aislop)
...
**交叉引用**: §硬规则-规则20(P0验证规格化, L527) | §硬规则-规则7(失败替换+3层重试) | §反模式-7(单Agent盲信) | §Agent Safety-Three-Layer Defense(L1048)
...
**失败动作**: pantheon-fix回归门控失败→回滚worktree, 项回STEP 2。MMAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L1005)。PAR不可用→跳过(MMAR兜底)。对抗验证发现新P0→追加pending, 本轮干净轮清零。

**交叉引用**: §STEP 7-双向验证(L993) | §STEP 6-分支路由(pantheon-fix, L932) | §STEP 7-降级模式(MMAR, L1005) | §外部加速Skill(8挂点, L78)
...
**交叉引用**: §迭代自检(20项完整定义, L570) | §24 收敛阈值自调整(干净轮计数器, L546) | §26 无意义输出循环阻断(L555) | §转移包§六(增量验证+审查门控) | §转移包§八-Q2(5层验证体系)
...
**失败动作**: 用户拒绝或Codex拒绝→回STEP 2重方案。用户批准+Codex不可用→降级模式(STEP 5降级条款: 用户单批准可执行, §规则1双批准门控 L502)。

**交叉引用**: §规则1双批准门控(L505) | §STEP 4-Codex审查 | §STEP 5-门控+用户批准 | §错误处理-降级模式(STEP 5行)

### 管线硬性约束

- **L1为干净轮前置条件**: hooks.test全绿 + 自检#21 Scope无遗漏 + 自检#22 无注入风险——三者缺一→该轮不计干净轮(§24 收敛阈值自调整 L543)。自检#21+#22完全内化, 无不可用风险, 不阻塞
- **L1-L3为L4输入**: 只有通过前置层的变更才进入清单核对——跳过前置层的清单核对=形式主义
- **L5不可跳过**: 迭代模式规则15暂停规则4(逐项反馈), 但P0安全敏感修复仍需用户中断确认(§15 迭代不中断 L534)
- **外部skill回退**: pantheon/MMAR/PAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L1005); Quadruple不安装→本表内化映射
...
**交叉引用**: §STEP 7-双向验证(L990) | §外部加速Skill(L78) | §20 P0验证规格化(L539) | §迭代自检#10(5源深度利用)
...
15. **迭代不中断** (v4.7.7 恢复完整定义): 迭代模式下持续执行不停顿。**停止例外**: (a)零bug收敛达标 (b)用户显式stop (c)致命错误无法继续 (d)预算完全耗尽(>90%+BREAK_GLASS, 见§修复预算)。此规则优先于规则1(双批准门控)、规则4(逐项反馈)、规则9(代码不符回滚)和规则10(方案锁定)——但不覆盖规则2(P0逻辑变更仍需用户批准)和§31(f) L46(6项永不简化)中的安全护栏。安全关键的门控检查和P0逻辑变更即使在迭代模式下也不可跳过。turn结束时输出进度行"第N轮: X→Y→继"(自检#1例外d)。
...
计数器持久化: 随每份snapshot落盘(格式"干净轮计数: i/2"——见L168 snapshot模板, bearings STATE正则双格式容错); 恢复时缺失/不可辨→按0处理(安全方向: 宁多扫一轮, 不误宣收敛)。
**turn-end统一决策** (v4.7.8, 权威——in-turn与"继"恢复共用, 消除双规范分叉): ①pending非空→继续修复; ②否则干净轮<2→启动下一轮扫描; ③否则未跑最终验证→启动50 Agent最终验证; ④50 Agent确认P0=0→写goal-reached-{版本}.md→输出签收单(首行"## 签收单", 见L185格式)→同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2, 含哨兵行)→停(此"停"仅在④全条件满足时有效——不可被①②③路径借用, v4.7.9-r6, 审计#4)。⑤ ②/③不可执行(三硬门全阻断/Agent启动全失败/预算不足)→写snapshot(nextAction保留当前状态)→输出对应状态进度行(饱和=⚡格式含/compact, 预算=停格式, PERMANENTLY_FAILED=第N轮[FAIL]→停)——进度行后停止, 不强行发射(v4.7.9-r6, 审计#6)。修复应用→干净轮清零→回②。§一字恢复四分支(①-④)=本决策的nextAction快捷入口, 冲突时以本决策为准。
...
- **安全护栏**: 详见§31(f) L46——6项永不简化。即使梯级1-6认为可以简化, 安全代码不可移除。
