...
55%饱和信号触发:
  ...
  4. 输出: "⚡ 55% — /compact + 继"
  5. 立即终止turn — 不echo、不确认、不Read刚写的文件
...
**恢复触发词** (v4.7.5新增):
- "继" / "继续执行" / "继续迭代" / "继续优化" / "继续修复" → 读最新snapshot → 恢复执行
- "resume" / "go on" / "pick up" → 同上
- "继续" (裸词，不含后缀) → 不触发恢复 → 提示用户明确意图("是说继续修复? 继续迭代? 还是继续等Codex?")
- "继续等" / "继续等待" / "继续等Codex" → 保持静默等待，不触发恢复路由
- snapshot不存在 → "无活动快照可恢复。开始新任务吗?"
...
- 4 Agent 精简扫描保证恢复后文件完整性
- 恢复失败时从 git 恢复上一版本的 snapshot
...
阈值估算: versionVector × ~12% (每轮20 Agent ≈ 10-15%上下文消耗)。压缩后恢复时versionVector清零。

`# ponytail: 阈值估算是启发式的(无token计数API)，ceiling: versionVector × 12% (最坏情况高估约20%)，upgrade: CC公开token计数API时替换为精确百分比`
...
- **本机制 (v4.7.5)**: 事后预防——循环发生后通过一字恢复避免损失积累。诚实: 不能自主压缩+自动继续。能做到: 保存全部状态+一字恢复。优先执行顺序: 规则26(L2 事后阻断) > PreToolUse hook(L3 事后止损) > 一字恢复(L4 事后恢复)

`# ponytail: 无法自主压缩(CC架构限制)，ceiling: 一字恢复模型，upgrade: CC支持AI自主发送终端命令时`

<!-- 死循环两层防御 + 一字恢复: L1=规则26(事后阻断 v4.6.9), L2=PreToolUse hook(平台止损 v4.7.2), L3=一字恢复(保存+续跑 v4.7.5) -->

<!-- v4.7.5 诚实架构声明: CC AI 不能自主压缩+自动继续。full-auto 是伪功能。honest semi-auto = 一字恢复。 -->
...
### 启动 — 见规则5(§硬规则-Agent)和规则23(§硬规则-防卡顿并行启动)
