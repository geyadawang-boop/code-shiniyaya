...
AI: 读最新snapshot → 恢复全部状态 → 继续执行
     ...
     └─ 恢复决策(v4.7.8, 替代模糊的"从断点继续"; 分支①-④自上而下求值首中即行, nextAction与实际状态矛盾时以实际状态(pending/计数)为准):
        ...
        └─ ④ nextAction=verify OR 干净轮计数≥2 → 确认收敛(=按规则24跑最终50 Agent验证, P0=0方可)→写goal-reached-{当前版本}.md→输出签收单(首行"## 签收单", 见L353模板)+同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2); **goal-reached-{当前版本}.md已存在且含P0=0字段**→直接签收单——旧版本goal-reached*.md(无版本名/他版本)一律不作数, 不得短路最终验证(v4.7.9-r4: v4.0.2陈旧goal-reached.md曾在HEAD上构成旁路陷阱)
```
...
- hooks.test任一FAIL→禁止进入干净轮计数(前置条件, §硬规则-规则24 L531)

**交叉引用**: §硬规则-规则24(干净轮前置条件) | §自主迭代模式-item 1(预发射三硬门) | §迭代自检-6 | §v4.6.11-aislop(确定性预扫, L822)
...
**失败动作**: pantheon-fix回归门控失败→回滚worktree, 项回STEP 2。MMAR不可用→降级CC 6+ Agent自我验证(§STEP 7降级模式, L988)。PAR不可用→跳过(MMAR兜底)。对抗验证发现新P0→追加pending, 本轮干净轮清零。

**交叉引用**: §STEP 7-双向验证(L978) | §STEP 6-分支路由(pantheon-fix, L932) | §STEP 7-降级模式(MMAR, L988) | §外部加速Skill(8挂点, L70)
...
**交叉引用**: §迭代自检(20项完整定义, L555) | §硬规则-规则24(干净轮计数器, L531) | §硬规则-规则26(无意义输出循环阻断, L540) | §转移包§六(增量验证+审查门控) | §转移包§八-Q2(5层验证体系)
...
**失败动作**: 用户拒绝或Codex拒绝→回STEP 2重方案。用户批准+Codex不可用→降级模式(STEP 5降级条款: 用户单批准可执行, §硬规则-规则1 L490)。

**交叉引用**: §硬规则-规则1(双批准门控, L490) | §STEP 4-Codex审查 | §STEP 5-门控+用户批准 | §错误处理-降级模式(STEP 5行)
...
**交叉引用**: §STEP 7-双向验证(L978) | §外部加速Skill(8挂点, L70) | §硬规则-规则20(P0验证规格化, L527) | §迭代自检#10(5源深度利用)
...
**turn-end统一决策** (v4.7.8, 权威——in-turn与"继"恢复共用, 消除双规范分叉): ①pending非空→继续修复; ②否则干净轮<2→启动下一轮扫描; ③否则未跑最终验证→启动50 Agent最终验证; ④50 Agent确认P0=0→写goal-reached-{版本}.md→按L353模板输出签收单(首行"## 签收单")→同turn写最终snapshot(nextAction=verify, 干净轮计数: 2/2, 含哨兵行)→停(此"停"仅在④全条件满足时有效——不可被①②③路径借用, v4.7.9-r6, 审计#4)。⑤ ②/③不可执行(三硬门全阻断/Agent启动全失败/预算不足)→写snapshot(nextAction保留当前状态)→输出对应状态进度行(饱和=⚡格式含/compact, 预算=停格式, PERMANENTLY_FAILED=第N轮[FAIL]→停)——进度行后停止, 不强行发射(v4.7.9-r6, 审计#6)。修复应用→干净轮清零→回②。§一字恢复四分支(①-④)=本决策的nextAction快捷入口, 冲突时以本决策为准。
...
`# ponytail: 旧selftest缺失声明已被v4.7.6 Selftest双层门控取代(见§自应用差距-Selftest双层门控已定义, L1675)。保留此行作为历史锚点——ponytail对自己的judge.py执行selftest，code-shiniyaya现已对自身规则执行selftest。`
