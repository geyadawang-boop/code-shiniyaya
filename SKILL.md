...
metadata:
  ...
  agent-floor: 6 (diagnosis), 10 (Codex verify), 5 (reference scan)
  batch-floor: 4
  agent-cap: 50
  permissions:
    file-read: true
...
# code-shiniyaya v4.6.8 — ponytail全量集成+十Skill栈+Hook基础设施

v4.6.8 — ponytail全量集成。十Skill栈+Hook基础设施。20 Agent交叉验证通过。14 PARTIAL→12 FULLY+2 FIXED。10 ponytail不变短语全部存在。安全护栏6/6。onecheck 4/4子规则。STEP 1 grep所有调用者。LOC计量代理+benchmark来源。

编排CC与Codex之间标准化双向验证闭环的元编排Skill。**不修改代码** — 编排诊断、方案生成、双批准门控、执行验证。

## 开发Skill栈 (v4.6.8, 每次对话强制激活)

code-shiniyaya自身的开发/迭代/优化必须在以下6个skill全部激活的状态下进行。缺失skill→先安装再继续。
...
**ponytail-gain基准来源** (v4.6.8): 5个日常任务(邮箱验证器/debounce/CSV求和/倒计时器/速率限制器) × 3个模型(Haiku/Sonnet/Opus)。每任务分别用"无ponytail"与"ponytail"生成方案, LLM裁判(temperature=0)按公开rubric评分。原始数据见 judge.py + tasks.py。

**ponytail LOC计量代理** (v4.6.8): 源代码文件数 + 源代码LOC(排除测试文件/配置文件/文档/构建产物)。仅计算非测试源代码的实际行数变化。
...
- **多级配置解析** (v4.6.8): env var > config file > hardcoded default。环境变量最高优先级, config.json的字段次之, 硬编码默认值兜底。适用: code-shiniyaya自身的任何可配置参数(如静默阈值N/批大小/预算比例)应采用同样三级解析。
...
- **子Agent规则集注入** (v4.6.8): SubagentStart hook将规则集注入子Agent。环境变量用正则筛选目标Agent类型。无效正则→fail-open(注入所有)。适用: code-shiniyaya的multi-agent-shiniyaya子Agent启动时应选择性注入规则。
...
### 架构依赖 (文档化但需CC平台能力升级)
以下模式已完整设计和验证，但在当前CC CLI请求-响应模型下无法自主激活。依赖: 事件循环/持久后台状态/跨会话连续性/自主任务通知处理。

- 规则15-17, 21-25: 自主迭代循环/双轨修复/趋同检测/Fast-fail守卫
- 反模式10-23(除18): 迭代相关的反模式
- 自检1/3/4/5/5b/7/8/9/10/11/12/14/15: 迭代模式自检
- STEP 8 Learn+Consolidate: 持久背景反思
- 终端信号协议(类型化返回值/工具调用完成信号): CC不向Agent注入虚拟工具
- 工作流上下文总线/CTX_UPDATE: 跨Agent调用无持久上下文
- 声明式事件依赖(listen_group): 无事件驱动引擎
- 代理工厂/动态指令闭包/工具预取: CC Agent框架层级不支持
...
**Codex通信模型**: 用户手动在CC和Codex之间复制粘贴消息。无API/自动集成。
...
4. **逐项反馈, stop优先, 迭代豁免** (v4.2.6): 每项→反馈→确认→继续。禁止批量。stop→立即停, 项=INTERRUPTED(已完成子步骤保留, 未开始子步骤待恢复)。唯一例外: STEP 6 P2微改动(<3行)可批量2项——P0/P1不适用。**迭代模式(规则15激活时): 规则4暂停——STEP 6执行时不要求逐项用户确认, P0/P1项自动执行+写入CHANGELOG.md审计轨迹, P2项可批量2项。用户显式中断(stop/CTRL+C)仍然优先。**

### Agent
5. **批次**: batch_size = max(4, min(24, cpu_cores-2)), cpu_cores=`python -c "import os; print(os.cpu_count() or 4)"`, 回退=4。总Agent上限=50(超出则排队复用)。迭代扫描20 Agent分2批: 第一批batch_size个, 剩余在首批首个完成后立即追加, 总并发窗口<60s。
...
20. **P0验证规格化** (v3.9.2): P0项的双Agent验证必须指定: 类型=investigator+general-purpose, 维度=正确性(代码逻辑)+编码安全(注入/越权/泄露)。不指定类型和维度的"2+ Agent验证"为空话。降级模式下P0验证升级为4 Agent(4类型各一): investigator+general-purpose+Plan+debugging——所有引用此规则处必须一致使用4 Agent规格。
...
26. **无意义输出循环阻断** (v4.6.8, 最高优先级, HARD ENFORCEMENT):
...
**实现**: 每个工具调用决策前，CC先在thinking中运行检查伪代码:
```
// Read阻断: 同一file+同一offset(±5行)→阻断, 不同offset允许
if (lastCalls.filter(c => c.file==thisCall.file && c.tool=="Read" && Math.abs(c.offset - thisCall.offset) <= 5).length >= 1) → BLOCK
// Grep阻断: 同一pattern+同一path→阻断
if (lastCalls.filter(c => c.tool=="Grep" && c.pattern==thisCall.pattern && c.path==thisCall.path).length >= 1) → BLOCK
// Bash wc -l阻断: 同一file→阻断
if (lastCalls.filter(c => c.tool=="Bash" && c.command.includes("wc -l") && c.command.includes(thisCall.file)).length >= 1) → BLOCK
// 确认词输出阻断: 上turn有确认词且无实质操作→阻断
if (lastOutput.match(/(done|final|complete|verified|confirmed)/) && lastTurnNoWrite && lastTurnNoAgent) → BLOCK
// Write↔Read done循环阻断
if (thisCall.file == "done" && lastCalls.some(c => (c.tool=="Read" || c.tool=="Write") && c.file=="done")) → BLOCK
```
...
5. **循环持续检查** (v4.3.3 分层): 分两阶段检查——(a) 先触发5b(2轮无新类别+CRITICAL不变→切未读文件), (b) 若5b不满足但3轮CR<20%或CRITICAL不降→切换Agent类型+维度优先级(规则24收敛标准: 发现数<5且连续2轮<10且均为P1/P2→触发50 Agent最终验证, 确认P0=0则目标达成)。达标→启动50 Agent最终验证确认。
...
18. **死循环根因阻断** (v4.6.5, HARD): CC追踪最后10个工具调用。检测: (a)Read-Grep-Read同信息≥2次→阻断; (b)Write done→Read done→Write done≥3次→阻断; (c)连续3次输出无≥1新信息→阻断; (d)输出内容hash与上次相同度>80%且无Write/Edit→阻断。阻断→写DEAD_LOOP_LOG.md→切换策略→等用户。此自检=规则26同级。

违反任何自检项=立即自纠正, 不等待用户, 不汇报。

### 交付 (v4.6.5 统一编号: 规则27-28)
...
| 预算监控 | 消耗率50-90% | 按级降级 | 50-75%→日志警告+P2降级; 75-90%→停止新P2+P1限1次; >90%→仅P0+Tier 2(每修复2 agent+交叉验证investigator+general-purpose) |
| STEP 4降级 | Codex N=5自动降级(验证阶段) | "Codex超时→降级" | STEP 4中止, 跳转STEP 5降级路径(用户单批准; P0 4 Agent验证) |
...
2. flush + fsync。fsync失败→重试1次(100ms)→仍失败→写入内存/`emergency-{ts}.json`并通知用户(State still preserved)。单次fsync失败仅通知用户, 保留tmp文件(不尝试写.write_failed——磁盘满时也会失败), 无额外文件写入。
...
| >90% | 仅 P0, 所有修复升级到 Tier 2, P1/P2延迟到backlog |
| Tier 2规格 | 每修复2次agent+交叉验证investigator+general-purpose | P0项最少验证规格 |
...
**降级模式**: Codex静默→N=4→"跳过?"→触发降级。session JSON中标注 `mode: degraded, degradedAt: "<ISO8601>"`。降级后STEP 5: 用户单批准(正常项+UNREVIEWED项); P0项仍需CC 4 Agent独立验证(investigator+general-purpose+Plan+debugging, 降级模式不豁免P0双验证)。STEP 7: CC 6+ Agent自我验证。降级为单向且不可逆——模式标志一旦设为degraded就不会恢复为normal。晚到Codex回复: 未执行项→STEP 4验证(提取新发现, 与CC已验证项对比), 已执行项→不重做但Codex发现记录在案供用户审查。冲突(Codex反对CC已执行项)→记录在DISPUTED_LOG.md, 用户裁决。
...
7. **grep所有调用者** (v4.6.8, STEP 1内部规则7, ponytail trace-before-fix): 对每个疑似bug(在目标源码仓中), grep该函数/方法的所有调用者, 确认影响范围后再提修复方案——修补共享函数一次(one guard in shared function is smaller diff than per-caller guard, 根因修复), 不修补仅被ticket命名的路径。
...
**P0双验证**: P0修复→即使Codex批准, CC也需4 Agent独立验证(规则20)。任一Agent异议→用户手动override。降级模式下同样适用——不因Codex缺失降低P0验证强度。
...
| 迭代扫描 | batch_size | Inv+GP | 并行, 无阶段门控 |
...
CC解析规则:
- `HANDOFF: investigator | found cross-file import chain in src/utils/` → 若investigator未达max_retries, 排队新investigator任务并注入handoff上下文。
- `HANDOFF: none | completed` → Agent完成, 不触发新任务。
- `HANDOFF: general-purpose | hit unknown pattern, falling back` → 使用general-purpose回退并附显式原因。

HANDOFF目标Agent的提示词注入上一Agent部分输出(最多800 tokens), 以 `[HANDOFF_CONTEXT from {source_agent}]` 为前缀。

### 跨Agent sender标签传播 (v3.9.14)

AutoAgent core.py(L323,437,621)中每条assistant消息携带`sender: agent.name`字段。code-shiniyaya扩展Agent输出标头:

```
AGENT: {agent_id}:{type}:{timestamp_iso}[->{handoff_target}]
```

- `{agent_id}`: 批内UUID前8字符
- `{type}`: investigator|general-purpose|Plan|debugging|caveman:cavecrew-*
- `->{handoff_target}`: 若Agent触发了HANDOFF→记录目标agent_id。若未触发→省略
- CC在去重合并(STEP 1.4)时保留所有sender标签, 形成完整溯源链: `discovery_agent → verify_agent → fix_agent`
- 溯源链写入WORKFLOW_LOG.md(路径: `{project_root}/.claude/memory/code-shiniyaya/`)用于审计: 每个finding可追溯到哪个agent组合产出的

### 多Agent批内反思注入 (v3.9.14)

AutoAgent fn_call_converter.py(L837-848)中连续assistant消息间注入空user消息防止强化循环。code-shiniyaya多Agent场景适用:

同批内若>=2 Agent连续产出的finding完全相同(file:line一致)→在下一轮Agent启动时自动注入反思提示: "Previous agents A and B both reported {finding}. Before adding another report, verify independently. If confirmed, explain what NEW evidence you found beyond what they already reported." 防止回声室效应导致同缺陷被重复计数。

### Handoff限额
- 每Agent最多触发1次handoff(防无限链)
- 同目标Agent类型最多接收2次handoff/批次(防过载)
...
### 分块批量写入 (v4.2.3, rag_tools.py batch_size=200模式)
批量操作（向量化/记忆写入/报告生成）超过单次上限时，按batch_size分块处理，每批独立添加，用chunk索引追踪。适用场景：40+ Agent的发现结果合并写入STEP4_TRIM_LOG时按200条/批分块，批间不超时。
...
5. **跨handoff上下文累加** (v3.9.14): Agent通过HANDOFF委托时, 当前workflow_context的摘要(诊断中的P0 bugs + 已修复项IDs, 最多1200 tokens)作为[HANDOFF_CONTEXT]注入目标Agent。目标Agent的CTX_UPDATE与源Agent的context合并(深度合并, 冲突时目标Agent优先)。Agent完成后的TERMINAL信号自动携带上下文快照hash, 用于跨handoff验证上下文未被篡改。

### CTX_UPDATE 安全防护 (P0强制)

Agent输出中的CTX_UPDATE注入存在伪造风险。强制的护措施:
...
def truncate_for_prompt(text: str, max_chars: int) -> str:
    ...
    tail = max(0, max_chars - head - 5)  # v4.6.6 fix: guard against negative tail
...
**protect_tools不可修改检测** [VAL] (edit_tools.py L27-29 protect_tools): 创建/删除工具前检查工具名是否已存在于registry——已存在则拒绝修改。[未实现] code-shiniyaya无等价protect_tools registry——config常量和核心规则当前无修改前校验机制。
...
**工作流XML Pydantic约束验证** [VAL] (worklow_form_complie.py): WorkflowForm使用Pydantic model_validator+field_validator进行多层约束: (a) 事件只能listen前面定义的事件→无循环依赖, (b) 输入数量=listen事件数量→数据流完整性, (c) RESULT输出最多1个→互斥分支保证, (d) GOTO action必须含value→非空检查。[未实现] scan-plan JSON和工作流上下文创建后无Pydantic/Jsonschema验证——当前仅依赖手工schema检查(CTX_UPDATE安全防护, 行801-809)。
...
**Markdown浏览器viewport分页抽象** [I/O] (abstract_markdown_browser.py): 所有浏览器统一实现address/viewport/page_up/page_down/visit_page/find_on_page接口——分页导航独立于底层获取方式(requests/Selenium/Playwright)。[部分实现] `paginate_large_output()`(行847-855)已实现分页模式，但多Agent输出未统一到抽象viewport接口。
...
**动态指令closure** [AGENT] (dummy_agent.py `instructions(context_variables)`): Agent指令不限于静态字符串——可以是一个接收运行时context_variables并返回动态生成指令的闭包函数。[未实现] code-shiniyaya当前使用固定提示词模板分配Agent任务，无动态指令生成。

**Agent工厂模式** [AGENT] (dummy_agent.py `get_xxx_agent(model: str) -> Agent`): 所有Agent通过工厂函数创建，model选择外部化——Agent定义与具体模型解耦。[未实现] code-shiniyaya的Agent类型与模型当前绑定——无外部化model参数的工厂函数。
...
**工具预取Meta-Agent** [AGENT] (tool_retriver_agent.py): 专用Agent在执行前预取+合并下游Agent所需的所有工具文档——将"需要什么工具"与"如何使用工具"分离。[未实现] STEP 0当前仅运行环境能力检测(行420-429), 无Agent类型/能力签名预取步骤。
...
**宿主重建索引+行限制** [MEM] (autodream autodream.msg.md): MEMORY.md索引由宿主(非Agent)从文件描述重建，有硬性行数限制——Agent管理内容，宿主管理索引。[未实现] code-shiniyaya的MEMORY.md无硬性行数上限，CC不自动重建索引。
...
**`{ts}`变量约定** (v4.6.8): 全文`{ts}`均表示当前ISO8601时间戳(如`20260716T143022`), 精确到秒, 不含冒号以兼容Windows文件名。首次使用在行315 learn-parse-error-{ts}.json。 最后一条消息content块注入`cache_control:{"type":"ephemeral"}`→重复系统提示词+历史前缀自动缓存→降低延迟和成本。需分别追踪cache_read_input_tokens和cache_creation_input_tokens(计费标准不同于常规input tokens)。适用: 迭代扫描中固定系统提示词和工作流上下文可启用缓存——20+ Agent扫描成本降低。
...
| 场景 | 步骤 | 说明 |
|------|------|------|
| P0 Bug | 1-7 | 10+ Agent + P0双验证 |
| 多文件修改 | 1-7 | 双向验证+符号分析 |
| P1/P2 Bug | 1-7 | 6+ Agent |
| 新功能 | 1-7 | OpenSpec可选 |
| 迭代扫描 | workflow | 20+ Agent; 参见 "迭代扫描工作流" |
| 注释/文档 | 1-2 | 跳过3-7 |
| 纯诊断 | 1-2 | 无Codex |
| 状态报告 | 3 | 仅格式C |
...
- stop/中断/CTRL+C → 立即停, 等下条消息
- 自主迭代(规则15优先于停止线): 规则15要求迭代不中断——工作流完成通知是继续信号而非停止信号, CC收到通知后立即处理+修复+启动下一轮。停止线"等下条消息"仅适用于用户显式中断(stop/CTRL+C), 不适用于迭代模式的自动流转。
- 输出即停止: CC若在迭代中途产生用户可见分析/汇报/状态文本→已停止迭代(违反规则15)。task-notification到达后唯一合法动作=静默处理journal.jsonl→应用修复→启动下一轮工作流。
...
### 启动 (无阶段门控)
- **Agent数量**: batch_size=max(4,min(16,cpu_cores-2)), 总Agent上限=50
- **关键**: 所有Agent在单一planar并行块中启动(`Promise.all`), 无phase()调用——不使用pipeline, 不使用阶段门控, 所有20 Agent同时并发启动。这避免了phase gate阻塞后续阶段的问题(一个慢agent阻止整个阶段推进)
- Agent类型选择: 文件扫描→caveman:cavecrew-investigator; 交叉引用→**永不使用Explore**(已知挂起率>10%); 逻辑/安全→general-purpose。所有类型均有general-purpose回退链
- CC标记Workflow工具返回的background_task_id用于手动终止

### 进度(完全基于log())
- 工作流log()事件自然流经对话。Agent启动时log一条, 返回结果时log一条
- CC在所有Agent完成后自然汇总——无需轮询、无需状态文件、无需定时器
- 落后检测: Agent启动后, 其他3+agent已完成但该agent无log()结果事件→落后→CC标记
- 停滞检测 (v4.2.6): 自主迭代模式→连续3轮迭代无新log()事件→工作流可能已停滞→CC自动检查(非用户可见输出, 仅内部判断); 若停滞→TaskStop+解析journal.jsonl+仅重跑未覆盖维度。交互模式→5+用户轮次无任何log()事件→触发同样检查。不写用户可见分析, 不等待用户确认。

### 恢复
- 用户说"终止"→CC用TaskStop(background_task_id)终止→解析journal.jsonl部分结果→仅重跑未覆盖维度(最多2次重试)
- 同slot重试2次均失败→永久跳过该维度
- 全部重试失败→系统级故障→写入ERRORS.md+自动启动替代方案(纯agent()模式, 同规模)。不写用户可见分析, 不等用户。
- **重连恢复** (v4.2.2, docker_env.py L82-95容器已存在→跳过创建模式): 工作流重启时检查前次状态——若上次工作流目录存在且journal.jsonl有部分结果→不从头开始, 直接从断点续跑(类似docker start跳过已存在容器)。检查顺序: (1)工作流目录存在? (2)journal.jsonl有type:"result"行? (3)scan-state-{iter}.json存在? → 任一为真→恢复模式, 不新建。
...
<!-- v4.6.8: ponytail全量集成完成。六Skill栈+Hook基础设施(9模式)。20 Agent交叉验证: 14 PARTIAL→12 FULLY+2 FIXED。10 ponytail不变短语全部存在。安全护栏6/6。onecheck 4/4子规则。STEP 1新增grep所有调用者(STEP 1内部规则7)。LOC计量代理+benchmark来源写入。-->
