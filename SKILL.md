...
1. **不中断+预发射** (v4.7.5 降级, v4.7.8消除干净轮卡顿, v4.7.9-r6 防伪发射): 同turn内工作流完成→立即处理结果→修复→**若未收敛→同turn内立即启动下一轮扫描Agent(不等用户"继")→然后输出进度行+等待结果**。若turn自然结束→输出"第N轮[dim]: X→Y→继"或"第N轮: 干净轮i/2→继"→等用户"继"→从snapshot恢复→继续下一轮。不可跨turn自主启动(CC架构: 无自主跨turn执行)。**关键**: 扫描结果返回且未收敛时——在同turn内启动下一轮扫描Agent(启动前过规则24三硬门: (i)饱和≥55%→走饱和优先线 (ii)预算剩余<20→走预算耗尽路径 (iii)有在飞/未处理结果→不重复发射先处理), Agent启动后才输出进度行结束turn。**反伪发射规则**(v4.7.9-r6, 审计实证: r4 turn写"第9轮在飞"但未调Workflow): Agent调用必须先于进度行——进度行是对"已实际发射Agent"的声明, 不是发射的替代品。同一turn无Workflow/Agent/Task工具调用记录却含"第N轮…→继"=伪发射——下次stop-guard v3.3 pre-launch门将block。预发射失败证明(API错误/Agent启动返回值)必须写入snapshot, 进度行引用之——不得裸输出进度行当作"已完成一轮"。`# ponytail: 预发射Agent结果跨turn可靠性=CC Agent生命周期保证, ceiling: Agent超时/CC重启→回退原"继→发射"路径, upgrade: CC保证跨turn结果可靠投递`
