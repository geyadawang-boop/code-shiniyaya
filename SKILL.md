...
# code-shiniyaya v4.7.7 — 一字恢复：诚实面对CC架构极限

v4.7.7 — honest compaction model. CC 请求-响应架构：Claude 不能给自己发消息、不能自主启动新 turn、不能在压缩后自动续跑。全自动做不到。能做到的：保存全部状态 → 用户只需打一个字"继" → 任务无缝恢复。fake full-auto that silently fails < honest semi-auto that reliably works.
...
| # | Skill | 级别 | 职责 |
|---|-------|------|------|
| 1 | **code-shiniyaya** | 编排器 | 9步闭环(STEP 0-8)+26条硬规则+19项自检+5源+ponytail七步阶梯+Hook基础设施 |
...
**根本限制** (v4.7.2, v4.7.7修正): 文本规则无法打破LLM验证强迫——模型进入吸引子状态时执行检查的认知能力已被循环劫持。可靠性排序: **L2平台阻断(echo-guard.js) > L3一字恢复 > L1文本规则**。模型侧最佳努力: 完成实质操作(Write/Edit/Agent启动)后立即停止turn，不确认状态、不echo确认词、不Read已写入文件。用户下一条消息时自然重置。

**外部看门狗** (v4.7.7, echo-guard.js v2.0): PreToolUse Bash hook `C:\Users\shiniyaya\.claude\hooks\echo-guard.js`，注册于 settings.json。实际机制: stdin JSON读取tool_input.command → (1)拦截无意义echo(echo done/ok/final/complete/verified/confirmed/纯数字/空串, 正则匹配) (2)同turn内同文件wc -l第2次→拦截 (3)8次Bash调用/turn上限 (4)30秒空闲窗口≈turn边界(启发式, 重置计数)。阻断=stdout返回`{decision:"block"}`，恒exit 0(静默失败契约——hook绝不冻结会话)。与规则26互补: hook管Bash层，规则26管Read/Grep/Write工具层transcript自查。注意: settings.json可能被其他插件重写——每次CC重启后确认PreToolUse条目仍存在。
...
```
55%饱和信号触发:
  1. 保存 memory/snapshot-{ts}.md (版本号/todo/关键数据)——原子写入(tmp+rename)+末尾哨兵行`<!-- SNAPSHOT-COMPLETE {ts} -->`
  2. CHANGELOG 追加一行
...
`# ponytail: 7步检查清单手动执行，ceiling: CC无法自动执行清单(需用户"继"触发)，upgrade: CC支持SessionStart hook自动执行时`
...
**为什么不在检测到55%时自动调`/compact`?**
- `/compact` 是终端命令，Claude 不能向自己发送终端命令
- `ScheduleWakeup` 是 `/loop` 动态节奏控制，不能发送压缩命令
- `CronCreate` 是独立的定时任务，不在当前会话上下文中运行
- 任何声称能绕过这些限制的方案 = 未经验证的伪功能
...
- 回退: 插件不可用(status报错/JSON-RPC不可解析/认证失效)→单次重试→仍失败→手动复制粘贴模型, silentMsgCount重置。消毒流水线(规则28)适用于手动粘贴路径; 插件JSON-RPC直传内容仍需STEP 4验证但跳过文本消毒(结构化传输无注入面)。静默阈值N=4仅手动模式适用。
...
20. **P0验证规格化** (v3.9.2, v4.6.9 明确模式区分): P0项的双Agent验证必须指定: 类型=investigator+general-purpose, 维度=正确性(代码逻辑)+编码安全(注入/越权/泄露)。不指定类型和维度的"2+ Agent验证"为空话。**非降级模式=2 Agent(investigator+general-purpose), 降级模式=4 Agent(investigator+general-purpose+Plan+debugging)**——所有引用此规则处必须根据当前模式使用对应Agent规格。模式区分见§错误处理-降级模式条款。
...
1. **工作流完成通知=继续信号, 用户可见消息=停止** (v4.7.5 标记NON-VIABLE): task-notification跨turn自主处理不可行(CC无自主通知处理能力)。降级替代: 用户"继"触发恢复→CC读取最近journal.jsonl→提取问题→应用修复→同turn内启动下一轮。CC若写了分析/汇报/状态文本→已违反规则15, 已停止。CC只能在(a)达标停止确认 (b)致命错误无法继续 (c)用户显式要求报告 (d)规则15要求的同turn结束进度行(仅限"第N轮: X→Y→继"格式) 这四种情况下输出用户可见消息。
...
19. **消毒流水线合规检查** (v4.7.7): STEP 3产出FOR_CODEX文本前自查规则28消毒6步已应用(Bidi→NFKC→零宽→Null→C0/C1→围栏转义); STEP 4解析Codex手动粘贴回复前对粘贴内容执行同一流水线(防Codex回复携带控制序列注入)。插件JSON-RPC直传内容豁免文本消毒但仍走STEP 4完整验证。
...
| 14 | **输出即停止** (v3.9.0, v4.7.7降级) | 工作流完成后CC写长篇分析/汇报→已停止迭代 | 唯一允许的用户可见输出=规则15进度行"第N轮: X→Y→继"; 长篇分析→写入报告文件而非对话 |
...
`# ponytail: 墙钟时间盒来自autoresearch TIME_BUDGET模式(program.md:23,108)，ceiling: Shell后台进程超时(echo-guard.js 30s/turn + 600s/workflow)，upgrade: CC支持子进程信号时`
...
触发 → 路由 (短语匹配优先于状态匹配):
  ...
  ├─ H类(自主迭代): "循环迭代"/"自主执行"/"优化skill"/"根据源文件优化" → 自主迭代模式(行312)
...
**回滚路径** (验证确认修复错误且已合并): 6.0模式→`git revert {commit}`(fix-log.tsv定位commit); 6.1模式→按pending项lastFileHash恢复原文件。回滚后项回STEP 2。无回滚规则=坏代码静默留存主分支，双向验证失效。
...
- **不可变检查清单**: scan-plan中的每项锁定创建时状态。Loop仅可翻转 `verified` 标志(boolean)。禁止修改 `task`/`source`/`dimension`字段。此不可变契约同样适用于pending-{id}.json中的待修复项——详见§待修复项不可变契约(行551)。
...
| 自检#18 死循环根因阻断 | §迭代自检-18 | 同turn内追踪工具调用+跨turn内容hash比对 | **部分可行** | 同turn内(a)(b)可行; 跨turn(c)(d)不可行(CC每turn fresh context) |
...
| 落后检测 | §迭代扫描工作流-FirstCompleted(行1437) | 3+ Agent完成，某Agent无log()事件 | **不可行** | 需要live log()事件流+跨Agent完成对比 |
...
`# ponytail: 旧selftest缺失声明已被v4.7.6 Selftest双层门控取代(行1505)。保留此行作为历史锚点——ponytail对自己的judge.py执行selftest，code-shiniyaya现已对自身规则执行selftest。`
