...
# code-shiniyaya v4.7.3 — 上下文感知防饱和 + 压缩前5 Agent完整性验证

v4.7.3 — context-aware loop prevention. 死循环根因=上下文窗口压力→验证强迫吸引子→echo循环。事后阻断(PreToolUse hook)=治标。事前预防(主动保存记忆→检测饱和→按需压缩)=治本。锚定到0不可利用模式。
...
### 架构依赖 (文档化待平台升级, 运行时可行性已验证: 见行~1350)
...
10/20项标记NON-VIABLE, 其余标记待激活。详细机制可行性矩阵: 见运行时可行性审计(行~1345)。
...
4. **提示用户压缩**: 输出一行 "上下文趋近饱和——建议发送 /compact 释放窗口。记忆已保存至 memory/snapshot-{ts}.md。"

用户发送 /compact → 新会话恢复时从 memory/snapshot-{ts}.md 读取状态继续。
...
|------------|------|
| <60% | 正常，无动作 |
| 60-80% | 保存memory，标注"建议压缩" |
| 80-95% | 保存memory + CHANGELOG + git提交，警告"即将饱和" |
| >95% | 保存一切，输出"/compact now"，不启动新Agent |

阈值由session JSON的versionVector估算(CC不暴露token计数API): 每轮迭代≈20 Agent调用≈消耗10-15%上下文。versionVector≥6→建议压缩，≥10→强制建议。
...
- **本机制**: 事前预防——循环发生前通过主动保存+压缩避免饱和。优先执行顺序: 事前预防 > 事后阻断(规则26) > 事后止损(PreToolUse hook)
...
<!-- 死循环三层防御: L1=上下文感知防饱和(事前预防 v4.7.3), L2=规则26(事后阻断 v4.6.9), L3=PreToolUse hook(平台级止损 v4.7.2) -->
...
## 停止线 — 见规则12-15(行217-224)、降级模式(行484)和自主迭代模式(行142)
...
### 启动 — 见规则5(行151 batch_size + Agent上限)和规则23(行231 Promise.all + 串联await)

### 进度 — 见自检#11(落后检测, ~行~283)和停滞检测(行~1341, 运行时可行性矩阵标记部分可行)

### 恢复 — 见规则7(重试+PERMANENTLY_FAILED)、错误表(迭代扫描行~317+Agent卡住>300s)、重连恢复(docker_env.py L82-95容器已存在→跳过创建模式, 行~1348)
