...
**干净轮前置条件** (v4.7.8): 本轮扫描Agent按自检#6规格全部成功返回(失败/TIMED_OUT槽位经规则7处理后再计); scan-state `scannedFiles`与上轮重叠<80% 或显式标注"轮换空间已尽", 否则该轮不计(覆盖盲区→自检#15轮换排空检测); 计入前hooks.test.js全绿(用例数随版本增长, v4.7.9=21)+agent-lint分数不降(机器证据附进度行)。"无结果到达"≠"0发现"——预发射批次死亡(API错误/无TERMINAL)→该轮不计数, 按item 7报告失败+规则7重启该批。**规模归一** (v4.7.9, autoresearch BPB思想): 降级轮(仅P0规格2-4 Agent)零发现不计干净轮——证据规模不足; 进度行标注本轮规模("第N轮[20A]: 干净轮i/2")。
...
**输入** (注入LLM): workflow_context_bus 快照 (diagnosis + plan + codex + execution + meta), 现有记忆文件 (最多24个, 每个≤2500字符, 60/40截断), MEMORY.md 索引 (≤6000字符)。**自产工件排除** (v4.7.9, autodream L582-584/L1119 tag-and-filter): 扫描/反思输入排除本skill自产工件(memory/、报告/、snapshot-*、CHANGELOG)——读取端按来源过滤, 事前预防自消费回授, 而非依赖事后去重
