...
55%饱和信号触发:
  ...
  4. 输出: "⚡ 55% — /compact + 继"
  5. 立即终止turn — 不echo done/ok/final/complete等确认词，不Read刚写的文件，不调用新Agent。输出仅允许饱和警告单行(步骤4)。
...
1. **工作流完成通知=继续信号, 用户可见消息=停止** (v4.7.5 标记NON-VIABLE): task-notification跨turn自主处理不可行(CC无自主通知处理能力)。降级替代: 用户"继"触发恢复→CC读取最近journal.jsonl→提取问题→应用修复→同turn内启动下一轮。CC若写了分析/汇报/状态文本→已违反规则15, 已停止。CC只能在(a)达标停止确认 (b)致命错误无法继续 (c)用户显式要求报告 这三种情况下输出用户可见消息。
...
3. **不静默检查** (v4.7.5 降级): CC是否在工作流完成后退出了? 若退出→等用户"继"触发恢复→读取最后journal.jsonl+应用修复+启动下一轮。
4. **工作流存活检查** (v4.7.5 降级): 上一个工作流是否被kill/崩溃/超时? 若是→用户"继"恢复时解析journal.jsonl部分结果→应用已确认的修复→仅重跑失败维度。
