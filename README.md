...
| **L2** | PreToolUse echo-guard.js v4.1（平台止损） | token-array Set 数据结构 destruct-vet(grep/rg/cat/head/tail/wc/diff/uniq/stat 无条件豁免; find/sort 先豁免后检查破坏性 flag)+8 次/turn 上限+跨 turn 指纹升级阶梯+command-context-aware(find -o=逻辑OR vs sort -o=输出文件)。CC 重启已验证 |
...
**v4.7.10** — v4.7.9: 一字恢复+三hook平台防御(echo-guard v4.1+stop-guard v3.4+bearings v3.0)+预发射防卡顿+干净轮收敛+autoCompactThreshold=55。v4.7.10: 转移包全量落地——30条硬规则(含规则29契约前置TDD/规则30全站一致性审计)+五层验证管线L1-L5+headroom集成+aislop 138基线+agent-lint 51/100基线+token审计。hooks.test 38/38。
