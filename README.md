...
- [2. 快速部署](#2-快速部署)
- [3. 📖 快速上手](#3-快速上手)
- [4. 三层防御 Hook 详解](#4-三层防御-hook-详解)
- [5. 30 条硬规则](#5-30-条硬规则)
- [6. 五层验证管线](#6-五层验证管线)
- [7. 9步闭环与5层管线映射](#7-9步闭环与5层管线映射)
- [8. 自主迭代模式](#8-自主迭代模式)
- [9. 上下文防饱和与一字恢复](#9-上下文防饱和与一字恢复)
- [10. 触发词表](#10-触发词表)
- [11. 外部加速 Skill](#11-外部加速-skill)
- [12. 防御栈版本表](#12-防御栈版本表)
- [13. 仓库结构](#13-仓库结构)
...
本指南演示安装完成后，从输入触发词到完成首次验证的端到端工作流。以最核心的触发词 **"双重检验"** 为例。
...
系统匹配 B 类触发词（双向验证类），自动进入 code-shiniyaya 9 步闭环（STEP 0–8）。

### 9 步闭环分步说明

**STEP 0 — 环境检测**
SessionStart Hook（bearings.js）检查 hook 注册状态、工作目录、git HEAD。输出类似：
```
[BEARINGS] echo-guard v4.3+stop-guard v3.5+bearings v3.0-r9 全挂载
[BEARINGS] STATE: v4.7.10 | 干净轮 0/2 | pending: 0 | hooks: 3/3
```

**STEP 1 — 预扫描（L1 静态检测）**
自动执行三合一快速检查：
- `npx aislop .` — 50+ 反模式规则扫描（亚秒级），仅报告 HIGH/CRITICAL
- `npx agent-lint score SKILL.md` — Skill 元质量分数，对比上轮基线
- `node references/hooks.test.js` — 42 回归用例全绿

三项全通过 → 进入 STEP 2；任一失败 → 修复后重试。

**STEP 2 — 深度诊断（L2 AI 初审）**
并行启动 6+ Agent：
- 正确性 Agent × 2（investigator + general-purpose）
- 编码安全 Agent × 2（investigator + general-purpose）
- 规则合规 Agent × 1（Plan 类型）
- 过度工程 Agent × 1（ponytail-review）

各 Agent 独立扫描源码和 SKILL.md，返回发现列表。全部完成后汇总给 STEP 3。

**STEP 3 — 方案生成（L2 输出阶段）**
汇总 Agent 发现 → 生成修复方案。同时输出 Codex 可复制文本（纯文本、无 Markdown 表格、`-->` 替代 `→`），供 Codex 独立读取。

**STEP 4 — Codex 双向验证（L3 对抗验证）**
方案以可复制文本格式发给 Codex 独立审查。Codex 返回每项 file:line 级别的验证结论。不一致项标记为争议点，进入 STEP 5 供用户裁决。

**STEP 5 — 双批准门控（L5 人工核验）← 你需要在此操作**
系统暂停执行，等待用户审查方案摘要和 Codex 验证结论：

```
══════════════════════════════════════════
  已生成修复方案，请审查后批准执行
  ─ 方案摘要: {简要描述}
  ─ Codex 结论: {通过 / 提出 N 项异议}
  ─ 争议点: {如有}
══════════════════════════════════════════
```

**你需要做什么**：阅读方案摘要和 Codex 结论，确认准确无误后，回复任意批准信号（如 `批准`、`同意`、`执行`、`ok`）。如果 Codex 已批准且你批准 → 系统进入 STEP 6 执行。如果仅你批准（Codex 不可用时）→ 降级为单批准执行。如果拒绝 → 系统回 STEP 2 重方案。

**STEP 6 — 执行**
批准后自动执行修复（Write / Edit 工具调用）。执行前自动触发 L1–L3 强制门控（行号校验、规则合规性、变更前快照）。Write/Edit 成功后自动 `git commit`。

**STEP 7 — 双向验证（L3 对抗验证）**
变更后再次验证：
- pantheon-fix — 多候选修复 + 回归门控（用户 opt-in）
- MMAR — 跨模型对抗审查（Codex 降级替代）
- Codex 复审 — 确认修复正确（如可用）
- 不一致项标记为 VARIANTS 报告

**STEP 8 — 元反思 + 知识合并**
本轮经验写入 `memory/` 知识库，CHANGELOG 追加记录，输出进度行。

### 终端输出模拟

以下是输入 `双重检验` 后终端会看到的典型输出：

```
[BEARINGS] echo-guard v4.3+stop-guard v3.5+bearings v3.0-r9 全挂载
[BEARINGS] STATE: v4.7.10 | 干净轮 0/2 | pending: 0 | hooks: 3/3

双重检验 → 匹配 B 类触发词，进入 9 步闭环
────────────────────────────────────────────

STEP 0: 环境检测通过 ✓
STEP 1: 预扫描...
  aislop .         → 0 HIGH, 3 LOW (已知)
  agent-lint score → 8.5/10 (较基线 -0.1, 继续观察)
  hooks.test.js    → 42 passed
STEP 2: 深度诊断 → 6 Agent 并行启动...
  ✓ 正确性 Agent: 无异常
  ✓ 编码安全 Agent: 无异常
  ⚠ 规则合规 Agent: 规则 17 覆盖不足
  ⚠ ponytail-review: SKILL.md L1432 空壳段落可删除
STEP 3: 方案生成完成
STEP 4: Codex 验证...
  ✓ Codex: 2 项确认，无新增争议点
...
══════════════════════════════════════════
  已生成修复方案，请审查后批准执行
  ─ 方案摘要: 规则 17 覆盖补充 + L1432 空壳删除
  ─ Codex 结论: 2 项均确认，无新增争议点
══════════════════════════════════════════

# 此处等待用户输入批准信号 #
> 批准

STEP 5: 用户批准 ✓ | Codex 验证 ✓ → 执行
STEP 6: 执行中... Edit × 2, Write × 1
STEP 7: 反向验证 ✓ | L3 对抗验证通过 ✓
STEP 8: 知识合并 → CHANGELOG 更新

第1轮[规则17+L1432]: 2→2→继
```

> **要点**：整个流程中，你只需在 STEP 5 审查方案并回复"批准"。其余步骤由系统自动完成。
...
### 4.1 echo-guard.js v4.3 — PreToolUse Bash 钩子
...
### 4.2 stop-guard.js v3.5 — Stop 钩子
...
### 4.3 bearings.js v3.0-r9 — SessionStart 钩子
...
9 步闭环（STEP 0–8）和五层验证管线（L1–L5）是 code-shiniyaya 的两个核心概念。**它们不是独立的两个系统——管线嵌套在闭环之内**。
...
│             ▼        ▼         ▼        ▼        ▼                │
│           L1       L2        L2       L3       L5                 │
│         静态检测    AI初审    AI初审   对抗验证  人工核验          │
│                                                                   │
│  关键理解：管线是嵌入闭环的质量门，不是并行的另一个流程              │
└──────────────────────────────────────────────────────────────────┘
```
...
| 闭环步骤 | 管线层级 | 说明 |
|---------|---------|------|
| STEP 0 环境检测 | 管线外 — 前置检查 | bearings.js 检查 hook 注册 / 工作目录 / git HEAD |
| STEP 1 预扫描 | **L1 静态检测** | aislop + agent-lint + hooks.test，零 LLM 成本的确定性检查 |
| STEP 2 深度诊断 | **L2 AI 初审** | 6+ Agent 并行扫描，覆盖正确性 / 编码安全 / 规则合规 / 过度工程 |
| STEP 3 方案生成 | **L2 AI 初审** | Agent 产出的汇总与方案化，属于 L2 的合成阶段 |
| STEP 4 Codex 验证 | **L3 对抗验证** | Codex 独立交叉检查，防御 CC 单模型盲区 |
| STEP 5 双批准门控 | **L5 人工核验** | 用户审查 L1–L4 证据后批准；Codex 验证辅助决策 |
| STEP 6 执行 | 管线外 — 执行阶段 | 执行变更，执行前自动触发 L1–L3 强制门控 |
| STEP 7 双向验证 | **L3 对抗验证** | 变更后的再次验证（pantheon-fix / MMAR / PAR） |
| STEP 8 元反思 | 管线外 — 后续整理 | 经验写入 memory/，不涉及验证门控 |

### L4 的特殊位置

L4（清单驱动自检）没有绑定到特定 STEP。它在 L3 之后、L5 之前由 CC 内化执行，**贯穿 L1–L5 的全周期核查**。每一轮变更都必须通过 L4 的"28 规则 ↔ 20 自检"映射检查。

### 为什么这样设计

- **9 步闭环**决定 **做什么**：先扫哪里、谁来验证、何时批准，是宏观工作流的编排时序。
- **五层管线**决定 **多严才能过**：每道门用什么标准、失败后回退到哪一步，是微观质量门槛。

没有管线，闭环缺少质量门槛——扫描结果无人把关。没有闭环，管线缺少编排时机——不知道何时该跑 L1、何时该跑 L3。两者缺一不可，嵌套不可分离。
