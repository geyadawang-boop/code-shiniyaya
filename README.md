...
| **selftest 双层门控** | ponytail | Layer 1 离线（免 API）+ Layer 2 LLM 裁判 |
...
| # | Skill | 级别 | 职责 |
|---|-------|------|------|
| 1 | code-shiniyaya | 编排 | 9 步闭环 + 26 规则 + 18 自检 |
...
**v4.7.9** — v4.7.7-v4.7.9: 一字恢复+三hook平台防御(echo-guard v3.4+stop-guard v3.3+bearings v3.0)+预发射防卡顿+干净轮收敛+autoCompactThreshold=55平台自动压缩。hooks.test 30/30。
