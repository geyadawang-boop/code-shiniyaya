...
**v4.7.10** — v4.7.9: 一字恢复+三hook平台防御(echo-guard v4.3+stop-guard v3.5+bearings v3.0)+预发射防卡顿+干净轮收敛+autoCompactThreshold=55。v4.7.10: 转移包全量落地——30条硬规则(含规则29契约前置TDD/规则30全站一致性审计)+五层验证管线L1-L5+headroom集成+aislop 138基线+agent-lint 51/100基线+token审计。hooks.test 42/42。
